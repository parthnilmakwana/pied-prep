  ...
  questions: [
    {
      ...
      constraints: ["Use bcrypt for hashing", "Email must be unique"],
      hints: ["Create a mongoose User schema first", "Check if user already exists before creating"],
      ...
      optimizedSolution: "app.post('/api/register', [body('email').isEmail()], async (req, res) => {\n  // Handle validation errors\n  // Check for existing user, hash with salt round 12\n  // save and respond\n});",
      commonMistakes: ["Storing plaintext passwords", "Missing input validation"],
      followUp: ["How to handle rate limiting for this route?", "How would you implement email verification?"]
    },
    {
      id: "backend-code-2",
      title: "Login API",
      difficulty: "Medium",
      topics: ["Node", "Express", "JWT"],
      ...
      constraints: ["Use bcrypt.compare for passwords", "Token should expire in 1h"],
      hints: ["Find the user by email first", "Sign the payload with a secret key"],
      ...
      commonMistakes: ["Returning different errors for wrong email vs wrong password (reveals existing users)"],
      followUp: ["How to implement refresh tokens?"]
    },
    {
      id: "backend-code-3",
      title: "Logout API",
      difficulty: "Easy",
      topics: ["Node", "Express", "Auth"],
      ...
      constraints: ["Handle both cookie clearing and optional token invalidation"],
      hints: ["Use res.clearCookie for cookie-based auth"],
      ...
      commonMistakes: ["Forgetting to handle client-side token removal if not using cookies"],
      followUp: ["How do you handle JWT revocation efficiently?"]
    },
    {
      id: "backend-code-4",
      title: "JWT Authentication",
      difficulty: "Medium",
      topics: ["Express", "Middleware", "JWT"],
      ...
      constraints: ["Handle missing tokens", "Handle expired tokens"],
      hints: ["Extract token from 'Bearer <token>' format"],
      ...
      commonMistakes: ["Crashing the server on jwt.verify failure by not using try-catch"],
      followUp: ["How to deal with role based access control in this middleware?"]
    },
    {
      id: "backend-code-5",
      title: "Refresh Token",
      ...
      constraints: ["Refresh tokens should be stored securely (DB or Redis)"],
      hints: ["Verify the refresh token against the DB first"],
      ...
      commonMistakes: ["Using the same secret for both access and refresh tokens"],
      followUp: ["What is Refresh Token Rotation?"]
    },
    {
      id: "backend-code-6",
      title: "Forgot Password",
      difficulty: "Medium",
      topics: ["Node", "Express", "Email"],
      ...
      constraints: ["Token must expire (e.g., in 15 minutes)"],
      hints: ["Use crypto.randomBytes for generating the token"],
      ...
      commonMistakes: ["Storing the token in plain text in the database", "Confirming if an email exists or not (security risk)"],
      followUp: ["Why should we hash the reset token in the database?"]
    },
    {
      id: "backend-code-7",
      ...
      constraints: ["Token must not be expired", "Clear token fields after successful reset"],
      hints: ["Hash the token from params to match the DB stored hash"],
      ...
      commonMistakes: ["Not clearing the reset token after successful usage"],
      followUp: ["What happens if two users request a password reset at the exact same time?"]
    },
    {
      id: "backend-code-8",
      ...
      hints: ["Verify the old password using bcrypt.compare before changing"],
      ...
      commonMistakes: ["Allowing changes without validating the old password"],
      followUp: ["How to invalidate other active sessions when a password is changed?"]
    },
    {
      id: "backend-code-9",
      title: "Upload Avatar",
      difficulty: "Medium",
      topics: ["Node", "Express", "Multer"],
      ...
      constraints: ["Only accept JPEG and PNG", "Max size 2MB"],
      hints: ["Configure Multer storage and fileFilter"],
      ...
      commonMistakes: ["Not validating the file type, allowing malicious script uploads"],
      followUp: ["How would you store these files in AWS S3 instead of local disk?"]
    },
    {
      id: "backend-code-10",
      ...
      constraints: ["Prevent mass assignment vulnerabilities"],
      hints: ["Extract only the allowed fields from req.body"],
      ...
      commonMistakes: ["Directly spreading req.body into the update query (Object injection)"],
      followUp: ["What is a Mass Assignment Vulnerability?"]
    },
    {
      id: "backend-code-11",
      title: "CRUD API",
      difficulty: "Medium",
      topics: ["Express", "MongoDB", "Mongoose"],
      ...
      constraints: ["Use appropriate HTTP verbs (GET, POST, PUT/PATCH, DELETE)"],
      hints: ["Create separate route handlers for each operation"],
      ...
      timeComplexity: "O(1) routing",
      spaceComplexity: "O(1)",
      solution: "const router = express.Router();\nrouter.route('/')\n  .get(async (req,res) => res.json(await Product.find()))\n  .post(async (req,res) => res.status(201).json(await Product.create(req.body)));\nrouter.route('/:id')\n  .get(async (req,res) => res.json(await Product.findById(req.params.id)))\n  .put(async (req,res) => res.json(await Product.findByIdAndUpdate(req.params.id, req.body, {new: true})))\n  .delete(async (req,res) => { await Product.findByIdAndDelete(req.params.id); res.sendStatus(204); });\napp.use('/products', router);",
      ...
      commonMistakes: ["Not checking if the item exists before updating or deleting (returns 200 instead of 404)"],
      followUp: ["How do you handle soft deletes?"]
    },
    {
      id: "backend-code-12",
      ...
      constraints: ["Default to page 1, limit 10 if not provided"],
      hints: ["Use Mongoose .skip() and .limit()"],
      ...
      commonMistakes: ["Executing countDocuments and find sequentially instead of Promise.all"],
      followUp: ["Why does .skip() become slow with large offsets?"]
    },
    {
      id: "backend-code-13",
      title: "Search API",
      difficulty: "Medium",
      topics: ["MongoDB", "Search"],
      ...
      constraints: ["Use regex or text indexes"],
      hints: ["Use $or operator with $regex in MongoDB"],
      ...
      commonMistakes: ["Using regex without considering performance impacts on large collections"],
      followUp: ["How do you optimize search in MongoDB?"]
    },
    {
      id: "backend-code-14",
      title: "Sorting API",
      ...
      constraints: ["Handle multiple sort fields"],
      hints: ["Mongoose .sort() accepts a string formatted exactly like the query param"],
      ...
      commonMistakes: ["Allowing users to sort by unindexed fields, leading to slow queries or crashes"],
      followUp: ["What happens when MongoDB tries to sort a large collection without an index?"]
    },
    {
      id: "backend-code-15",
      title: "Filtering API",
      difficulty: "Hard",
      topics: ["MongoDB", "Express"],
      ...
      constraints: ["Handle exact matches and inequality operators dynamically"],
      hints: ["Stringify req.query and replace operators with $ equivalents"],
      ...
      commonMistakes: ["Failing to strip out control parameters (page, sort) from the filter object"],
      followUp: ["What is NoSQL Injection and how does this approach mitigate or invite it?"]
    },
    {
      id: "backend-code-16",
      ...
      constraints: ["Assume req.user is already populated by auth middleware"],
      hints: ["Use a closure to pass roles to the middleware"],
      approach: "Create a function returning a middleware that checks if req.user.role is in the allowed roles array.",
      timeComplexity: "O(1)",
      spaceComplexity: "O(1)",
      solution: "const authorize = (...roles) => {\n  return (req, res, next) => {\n    if (!roles.includes(req.user.role)) {\n      return res.status(403).json({ error: 'User role not authorized' });\n    }\n    next();\n  };\n};\n\n// Usage:\napp.delete('/users/:id', authMiddleware, authorize('admin', 'superadmin'), async (req, res) => {\n  // Delete logic\n});",
      optimizedSolution: "const authorize = (...roles) => (req, res, next) => {\n  // Also verify user is active and their permissions haven't been revoked\n  // Check role hierarchy if applicable\n};",
      commonMistakes: ["Hardcoding 'admin' instead of passing an array of allowed roles"],
      followUp: ["How would you implement Attribute-Based Access Control (ABAC)?"]
    },
    {
      id: "backend-code-17",
      title: "Rate Limiter",
      difficulty: "Medium",
      topics: ["Express", "Security", "Redis"],
      ...
      constraints: ["Use a sliding or fixed window algorithm"],
      hints: ["Use the express-rate-limit library for a quick solution, or implement manually with Redis"],
      ...
      commonMistakes: ["Applying in-memory rate limiters in a multi-server clustered environment"],
      followUp: ["Why does in-memory rate limiting fail on scaled applications?"]
    },
    {
      id: "backend-code-18",
      title: "Email Verification",
      difficulty: "Medium",
      topics: ["Express", "Email", "Auth"],
      ...
      constraints: ["Tokens must be unique and expire"],
      hints: ["Similar logic to reset password, but updating a boolean flag instead"],
      ...
      commonMistakes: ["Failing to expire the verification token"],
      followUp: ["Should users be allowed to log in before verifying their email?"]
    },
    {
      id: "backend-code-19",
      ...
      constraints: ["Must not block the request lifecycle"],
      hints: ["Access req.method and req.originalUrl"],
      ...
      commonMistakes: ["Forgetting to call next(), which leaves the request hanging indefinitely"],
      followUp: ["Why is structured logging (like Winston) preferred over console.log in production?"]
    },
    {
      id: "backend-code-20",
      title: "Error Handling Middleware",
      difficulty: "Medium",
      topics: ["Express", "Error Handling"],
      ...
      constraints: ["Hide stack traces in production"],
      hints: ["Express error middleware takes 4 arguments: (err, req, res, next)"],
      ...
      commonMistakes: ["Placing error middleware before other routes", "Leaking stack traces in production mode"],
      followUp: ["How do you handle unhandled promise rejections outside of Express routes?"]
    }
  ]
};
