  ...
  questions: [
    {
      ...
      constraints: ["Assume normal collection size"],
      hints: ["Use the find() method without any filter."],
      ...
      commonMistakes: ["Using findOne instead of find when multiple documents are expected."],
      followUp: ["How would you limit the number of results returned?"]
    },
    {
      id: "mongo-code-2",
      title: "Find users older than 18",
      difficulty: "Easy",
      topics: ["MongoDB", "CRUD", "Filtering"],
      ...
      constraints: ["Assume `age` is a numeric field."],
      hints: ["Use the $gt operator for comparison."],
      ...
      commonMistakes: ["Using >= instead of > if strict greater than is required.", "Not using an index for large collections."],
      followUp: ["How to find users between 18 and 30?"]
    },
    {
      id: "mongo-code-3",
      title: "Sort users by age",
      difficulty: "Easy",
      topics: ["MongoDB", "Sorting"],
      ...
      constraints: ["Assume age is numeric"],
      hints: ["Use the sort() cursor method with -1 for descending."],
      ...
      commonMistakes: ["Sorting in memory exceeding the 32MB limit if no index is used."],
      followUp: ["How do you sort by multiple fields, e.g., age then name?"]
    },
    {
      id: "mongo-code-4",
      title: "Pagination",
      difficulty: "Medium",
      topics: ["MongoDB", "Pagination"],
      ...
      constraints: ["Collection can be very large"],
      hints: ["Use skip() and limit()."],
      ...
      commonMistakes: ["Using skip() for large offsets which is slow because the server still scans the skipped documents."],
      followUp: ["How would you implement cursor-based pagination?"]
    },
    {
      id: "mongo-code-5",
      ...
      constraints: ["Assume valid numeric ages"],
      hints: ["Use the $group stage with _id: null.", "Use $sum and $avg operators."],
      ...
      commonMistakes: ["Forgetting to group by null when aggregating the entire collection."],
      followUp: ["How do you filter documents before grouping?"]
    },
    {
      id: "mongo-code-6",
      title: "$lookup",
      difficulty: "Medium",
      topics: ["MongoDB", "Aggregation", "Joins"],
      ...
      constraints: ["Assume 1-to-many relationship"],
      hints: ["Use the $lookup stage."],
      ...
      commonMistakes: ["Not creating an index on the foreignField, leading to slow joins.", "Document size exceeding 16MB if a user has too many orders."],
      followUp: ["How do you handle cases where a user has thousands of orders?"]
    },
    {
      id: "mongo-code-7",
      title: "$group",
      ...
      constraints: ["Status is a string field"],
      hints: ["Use $group with _id: '$status'"],
      ...
      solution: "db.users.aggregate([\n  {\n    $group: {\n      _id: '$status',\n      count: { $sum: 1 }\n    }\n  }\n])",
      optimizedSolution: "db.users.aggregate([\n  {\n    $group: {\n      _id: '$status',\n      count: { $sum: 1 }\n    }\n  }\n])",
      commonMistakes: ["Using incorrect field path (missing '$' prefix for the _id field)."],
      followUp: ["How would you get the list of user names in each status group?"]
    },
    {
      id: "mongo-code-8",
      title: "$match",
      ...
      constraints: ["Place match early for performance"],
      hints: ["Use the $match stage."],
      ...
      commonMistakes: ["Placing $match after a $group or $project stage when the filter could be applied earlier, missing index usage."],
      followUp: ["Why is it important to put $match as early as possible in the pipeline?"]
    },
    {
      id: "mongo-code-9",
      title: "$project",
      ...
      constraints: ["Exclude _id explicitly"],
      hints: ["Use the $project stage."],
      approach: "Use the `$project` stage specifying `{ name: 1, email: 1, _id: 0 }`.",
      timeComplexity: "O(N)",
      spaceComplexity: "O(1) per document",
      solution: "db.users.aggregate([\n  {\n    $project: {\n      name: 1,\n      email: 1,\n      _id: 0\n    }\n  }\n])",
      optimizedSolution: "db.users.aggregate([\n  {\n    $project: {\n      name: 1,\n      email: 1,\n      _id: 0\n    }\n  }\n])",
      commonMistakes: ["Forgetting to explicitly exclude _id with _id: 0 if it's not wanted."],
      followUp: ["How can you create a new computed field using $project?"]
    },
    {
      id: "mongo-code-10",
      ...
      constraints: ["Must be fast for large collections"],
      hints: ["Use countDocuments() or estimatedDocumentCount()"],
      ...
      commonMistakes: ["Using .find().count() which is deprecated or slow."],
      followUp: ["When would you use estimatedDocumentCount vs countDocuments?"]
    },
    {
      id: "mongo-code-11",
      ...
      constraints: ["Order of fields in index matters"],
      hints: ["Equality, Sort, Range rule (ESR)."],
      ...
      optimizedSolution: "db.users.createIndex({ lastName: 1, firstName: 1 }, { background: true }) // Create in background for production",
      commonMistakes: ["Putting the sort field before the equality field in the index definition."],
      followUp: ["What is the ESR rule for indexing?"]
    },
    {
      id: "mongo-code-12",
      title: "Text Search",
      difficulty: "Hard",
      topics: ["MongoDB", "Search", "Indexing"],
      ...
      constraints: ["Requires text index"],
      hints: ["Use the $text operator and $search."],
      ...
      optimizedSolution: "db.articles.find(\n  { $text: { $search: 'mongodb' } },\n  { score: { $meta: 'textScore' } }\n).sort({ score: { $meta: 'textScore' } }) // Sort by relevance score",
      commonMistakes: ["Forgetting to create a text index before using $text.", "Not sorting by textScore."],
      followUp: ["How do you search for an exact phrase instead of individual words?"]
    },
    {
      id: "mongo-code-13",
      title: "Update many",
      ...
      constraints: ["Use appropriate update operator"],
      hints: ["Use updateMany() and $set."],
      ...
      commonMistakes: ["Using updateOne instead of updateMany.", "Forgetting the $set operator."],
      followUp: ["How do you increment a numeric field during an update?"]
    },
    {
      id: "mongo-code-14",
      title: "Delete many",
      ...
      constraints: ["Permanent deletion"],
      hints: ["Use deleteMany() and $lt."],
      ...
      commonMistakes: ["Accidentally running deleteMany({}) without a filter, wiping the collection."],
      followUp: ["What is a soft delete and how would you implement it instead?"]
    },
    {
      id: "mongo-code-15",
      title: "Transactions",
      difficulty: "Hard",
      topics: ["MongoDB", "Transactions"],
      ...
      constraints: ["Requires replica set"],
      hints: ["Use client.startSession() and session.withTransaction()."],
      ...
      commonMistakes: ["Forgetting to pass the `{ session }` object to the operations inside the transaction.", "Running transactions on a standalone instance (requires replica set or sharded cluster)."],
      followUp: ["What happens if a transaction takes too long?"]
    }
  ]
};
