  ...
  questions: [
    {
      ...
      hints: ["Since the array is sorted, we can divide the search space in half at each step."],
      ...
      commonMistakes: ["Not using Math.floor leading to decimal indices.", "Incorrect boundary updates (e.g. left = mid) leading to infinite loops."],
      followUp: ["What if there are duplicate elements and you need to find the first occurrence?"]
    },
    {
      id: "dsa-code-2",
      title: "Bubble Sort",
      ...
      examples: [{ input: "[8, 5, 2, 9, 5, 6, 3]", output: "[2, 3, 5, 5, 6, 8, 9]", explanation: "Array sorted in ascending order." }],
      constraints: ["1 <= arr.length <= 1000"],
      hints: ["Iterate through the array and swap adjacent elements if they are out of order. Repeat until no swaps are made."],
      ...
      commonMistakes: ["Not optimizing the inner loop boundary (elements at the end are already sorted).", "Failing to break early if no swaps occurred."],
      followUp: ["What is the best-case time complexity of the optimized version?"]
    },
    {
      id: "dsa-code-3",
      ...
      examples: [{ input: "[8, 5, 2, 9, 5, 6, 3]", output: "[2, 3, 5, 5, 6, 8, 9]", explanation: "Array sorted in ascending order." }],
      constraints: ["1 <= arr.length <= 1000"],
      hints: ["Find the smallest element in the unsorted portion of the array and swap it with the first unsorted element."],
      ...
      commonMistakes: ["Swapping elements on every comparison rather than only at the end of the inner loop."],
      followUp: ["Is Selection Sort stable?"]
    },
    {
      id: "dsa-code-4",
      ...
      examples: [{ input: "[8, 5, 2, 9, 5, 6, 3]", output: "[2, 3, 5, 5, 6, 8, 9]", explanation: "Array sorted in ascending order." }],
      constraints: ["1 <= arr.length <= 1000"],
      hints: ["Build the sorted array one element at a time by repeatedly taking the next unsorted element and inserting it into its correct position."],
      ...
      commonMistakes: ["Using repeated swaps instead of shifting elements, which is less efficient."],
      followUp: ["When is Insertion Sort preferred over Quick Sort?"]
    },
    {
      id: "dsa-code-5",
      title: "Merge Sort",
      ...
      examples: [{ input: "[5,2,3,1]", output: "[1,2,3,5]", explanation: "Array sorted in ascending order." }],
      constraints: ["1 <= nums.length <= 5 * 10^4", "-5 * 10^4 <= nums[i] <= 5 * 10^4"],
      hints: ["Divide the array in half, sort each half, and merge them back together."],
      ...
      commonMistakes: ["Creating too many new arrays during slice and concat, leading to memory issues in strict environments."],
      followUp: ["Can you implement an iterative version of Merge Sort?"]
    },
    {
      id: "dsa-code-6",
      title: "Quick Sort",
      ...
      hints: ["Pick a pivot element, partition the array around it, and recursively sort the left and right subarrays."],
      ...
      commonMistakes: ["Using naive O(N) memory approach (creating new arrays left/right) instead of in-place swapping."],
      followUp: ["How would you choose a better pivot to avoid O(N^2) worst case?"]
    },
    {
      id: "dsa-code-7",
      title: "Stack",
      ...
      examples: [{ input: "push(5), push(10), pop()", output: "10", explanation: "LIFO order: 10 was the last added, so it is popped first." }],
      constraints: ["0 <= val <= 10^5", "At most 10^5 calls will be made to push, pop, peek, and isEmpty."],
      hints: ["A stack follows Last-In-First-Out (LIFO) principle.", "An array can be used as the underlying data structure."],
      approach: "Use a Javascript array and its built-in push and pop methods to create a LIFO queue.",
      timeComplexity: "O(1)",
      spaceComplexity: "O(N)",
      solution: "class Stack {\n  constructor() {\n    this.items = [];\n  }\n  push(element) {\n    this.items.push(element);\n  }\n  pop() {\n    return this.items.pop();\n  }\n  peek() {\n    return this.items[this.items.length - 1];\n  }\n  isEmpty() {\n    return this.items.length === 0;\n  }\n}",
      optimizedSolution: "class Node { constructor(value) { this.value = value; this.next = null; } }\nclass Stack {\n  constructor() { this.first = null; this.last = null; this.size = 0; }\n  push(val) {\n    let newNode = new Node(val);\n    if (!this.first) { this.first = newNode; this.last = newNode; } else {\n      let temp = this.first;\n      this.first = newNode;\n      this.first.next = temp;\n    }\n    return ++this.size;\n  }\n  pop() {\n    if (!this.first) return null;\n    let temp = this.first;\n    if (this.first === this.last) this.last = null;\n    this.first = this.first.next;\n    this.size--;\n    return temp.value;\n  }\n}",
      commonMistakes: ["Not handling the pop operation when the stack is empty."],
      followUp: ["Can you implement a stack using a Linked List to guarantee O(1) resizing time?"]
    },
    {
      id: "dsa-code-8",
      title: "Queue",
      ...
      examples: [{ input: "enqueue(5), enqueue(10), dequeue()", output: "5", explanation: "FIFO order: 5 was added first, so it is removed first." }],
      constraints: ["0 <= val <= 10^5"],
      hints: ["A queue follows First-In-First-Out (FIFO) principle."],
      ...
      commonMistakes: ["Using Array.shift() for dequeue in JS, which causes O(N) performance per operation."],
      followUp: ["Implement a circular queue."]
    },
    {
      id: "dsa-code-9",
      title: "Linked List",
      ...
      examples: [{ input: "append(1), append(2), deleteWithValue(1)", output: "List: 2 -> null", explanation: "1 is removed." }],
      constraints: ["0 <= val <= 10^5"],
      hints: ["Keep track of the head (and optionally tail) pointer."],
      ...
      timeComplexity: "O(N)",
      spaceComplexity: "O(N)",
      solution: "class Node { constructor(val) { this.val = val; this.next = null; } }\nclass LinkedList {\n  constructor() { this.head = null; }\n  append(val) {\n    let newNode = new Node(val);\n    if(!this.head) { this.head = newNode; return; }\n    let current = this.head;\n    while(current.next) current = current.next;\n    current.next = newNode;\n  }\n  find(val) {\n    let current = this.head;\n    while(current) { if(current.val === val) return current; current = current.next; }\n    return null;\n  }\n}",
      optimizedSolution: "class Node { constructor(val) { this.val = val; this.next = null; } }\nclass LinkedList {\n  constructor() { this.head = null; this.tail = null; }\n  append(val) {\n    let newNode = new Node(val);\n    if (!this.head) { this.head = newNode; this.tail = newNode; return; }\n    this.tail.next = newNode;\n    this.tail = newNode;\n  }\n  deleteWithValue(val) {\n    if(!this.head) return;\n    if(this.head.val === val) { this.head = this.head.next; return; }\n    let current = this.head;\n    while(current.next && current.next.val !== val) current = current.next;\n    if(current.next) { current.next = current.next.next; if(!current.next) this.tail = current; }\n  }\n}",
      commonMistakes: ["Forgetting to update the tail pointer when deleting the last node."],
      followUp: ["How would you implement a doubly linked list?"]
    },
    {
      id: "dsa-code-10",
      title: "HashMap",
      difficulty: "Medium",
      topics: ["Data Structures", "HashMap", "Hashing"],
      problemStatement: "Implement a simple Hash Table with set, get, and keys methods using an array.",
      examples: [{ input: "set('pink', '#ffc0cb'), get('pink')", output: "'#ffc0cb'", explanation: "Stores and retrieves a key-value pair." }],
      constraints: ["Keys are strings"],
      hints: ["Use a hash function to convert a string key into an array index."],
      ...
      optimizedSolution: "class HashTable {\n  constructor(size=53) { this.keyMap = new Array(size); }\n  _hash(key) {\n    let total = 0, WEIRD_PRIME = 31;\n    for(let i=0; i<Math.min(key.length, 100); i++) {\n      total = (total * WEIRD_PRIME + key.charCodeAt(i) - 96) % this.keyMap.length;\n    }\n    return total;\n  }\n  set(key, value) {\n    let index = this._hash(key);\n    if(!this.keyMap[index]) this.keyMap[index] = [];\n    this.keyMap[index].push([key, value]);\n  }\n  get(key) {\n    let index = this._hash(key);\n    if(this.keyMap[index]) {\n      for(let i=0; i<this.keyMap[index].length; i++) {\n        if(this.keyMap[index][i][0] === key) return this.keyMap[index][i][1];\n      }\n    }\n    return undefined;\n  }\n}",
      commonMistakes: ["Not handling hash collisions."],
      followUp: ["How would you handle resizing the hash table?"]
    },
    {
      id: "dsa-code-11",
      title: "Two Pointers",
      ...
      examples: [{ input: "nums = [1, 2, 3, 4, 6], target = 6", output: "[1, 3]", explanation: "nums[1] + nums[3] = 2 + 4 = 6" }],
      constraints: ["2 <= nums.length <= 10^5", "Array is sorted in ascending order."],
      hints: ["Use a pointer at the beginning and a pointer at the end of the array."],
      ...
      commonMistakes: ["Failing to handle duplicate logic if indices need to be unique.", "Using nested loops (O(N^2)) instead of two pointers."],
    ...
    },
    {
      ...
      examples: [{ input: "arr = [2, 1, 5, 1, 3, 2], k = 3", output: "9", explanation: "Subarray with maximum sum is [5, 1, 3]." }],
      constraints: ["1 <= k <= arr.length <= 10^5"],
      hints: ["Instead of calculating the sum of every k-sized subarray from scratch, subtract the element going out and add the element coming in."],
      ...
      commonMistakes: ["Recalculating the sum of the entire window every time, resulting in O(N*K) time complexity."],
      followUp: ["What if k is not fixed but you need a target sum?"]
    },
    {
      id: "dsa-code-13",
      title: "Recursion",
      ...
      examples: [{ input: "n = 5", output: "5", explanation: "Fibonacci sequence: 0, 1, 1, 2, 3, 5" }],
      constraints: ["0 <= n <= 30"],
      hints: ["Base cases are n=0 -> 0, n=1 -> 1. Recursive step is F(n) = F(n-1) + F(n-2)."],
      ...
      commonMistakes: ["Missing base cases causing stack overflow.", "Not using memoization, resulting in Exponential time."],
      followUp: ["Can you do it iteratively in O(1) space?"]
    },
    {
      id: "dsa-code-14",
      title: "DFS",
      ...
      examples: [{ input: "graph = {A:['B','C'], B:['D'], C:['E'], D:[], E:[]}, start = 'A'", output: "['A', 'B', 'D', 'C', 'E']", explanation: "DFS traversal order." }],
      constraints: ["Graph has V vertices and E edges.", "1 <= V <= 1000"],
      hints: ["Use a recursion or a stack to explore as far down a branch as possible before backtracking."],
      ...
      optimizedSolution: "function dfsIterative(graph, start) {\n  let result = [], visited = {}, stack = [start];\n  visited[start] = true;\n  while(stack.length) {\n    let current = stack.pop();\n    result.push(current);\n    graph[current].forEach(neighbor => {\n      if(!visited[neighbor]) {\n        visited[neighbor] = true;\n        stack.push(neighbor);\n      }\n    });\n  }\n  return result;\n}",
      commonMistakes: ["Not keeping track of visited nodes leading to infinite loops in cyclic graphs."],
      followUp: ["How does the iterative output order differ from recursive?"]
    },
    {
      id: "dsa-code-15",
      title: "BFS",
      ...
      examples: [{ input: "graph = {A:['B','C'], B:['D'], C:['E'], D:[], E:[]}, start = 'A'", output: "['A', 'B', 'C', 'D', 'E']", explanation: "BFS traversal order." }],
      constraints: ["Graph has V vertices and E edges.", "1 <= V <= 1000"],
      hints: ["Use a queue to explore all neighbors of a node before moving to the next level."],
      ...
      timeComplexity: "O(V + E)",
      spaceComplexity: "O(V)",
      solution: "function bfs(graph, start) {\n  let queue = [start], result = [], visited = { [start]: true };\n  while(queue.length) {\n    let current = queue.shift();\n    result.push(current);\n    graph[current].forEach(neighbor => {\n      if(!visited[neighbor]) {\n        visited[neighbor] = true;\n        queue.push(neighbor);\n      }\n    });\n  }\n  return result;\n}",
      optimizedSolution: "function bfs(graph, start) {\n  // In a real scenario, use a real Queue class instead of Array.shift() for O(1) dequeue\n  let queue = [start], result = [], visited = new Set([start]);\n  while(queue.length) {\n    let current = queue.shift();\n    result.push(current);\n    for (let neighbor of graph[current]) {\n      if (!visited.has(neighbor)) {\n        visited.add(neighbor);\n        queue.push(neighbor);\n      }\n    }\n  }\n  return result;\n}",
      commonMistakes: ["Using Array.shift() in tight loops can cause performance drops on huge datasets."],
      followUp: ["How can BFS be used to find the shortest path in an unweighted graph?"]
    },
    {
      id: "dsa-code-16",
      ...
      examples: [{ input: "root = [1,null,2,3]", output: "[1,3,2]", explanation: "Inorder traversal visits Left, Root, Right." }],
      constraints: ["Number of nodes is in range [0, 100]"],
      hints: ["Traverse the left subtree, visit the root, traverse the right subtree."],
      ...
      optimizedSolution: "function inorderIterative(root) {\n  let res = [], stack = [], current = root;\n  while(current || stack.length) {\n    while(current) {\n      stack.push(current);\n      current = current.left;\n    }\n    current = stack.pop();\n    res.push(current.val);\n    current = current.right;\n  }\n  return res;\n}",
      commonMistakes: ["Confusing the order of InOrder, PreOrder, and PostOrder."],
      followUp: ["Can you implement PostOrder iteratively?"]
    },
    {
      id: "dsa-code-17",
      ...
      hints: ["A tree's depth is 1 + the max depth of its left and right subtrees."],
      ...
      optimizedSolution: "const maxDepth = (root) => root ? 1 + Math.max(maxDepth(root.left), maxDepth(root.right)) : 0;",
      commonMistakes: ["Returning 0 for a leaf node instead of 1, depending on if you are counting nodes or edges."],
      followUp: ["How would you do this iteratively using BFS?"]
    },
    {
      id: "dsa-code-18",
      ...
      hints: ["Use a stack. Push opening brackets. When encountering a closing bracket, pop the stack and check if they match."],
      ...
      commonMistakes: ["Not checking if the stack is empty at the end (e.g. input '[')."],
      followUp: ["What if there are other characters in the string besides brackets?"]
    },
    {
      id: "dsa-code-19",
      ...
      examples: [{ input: "strs = [\"flower\",\"flow\",\"flight\"]", output: "\"fl\"", explanation: "The common prefix is 'fl'." }],
      constraints: ["1 <= strs.length <= 200", "0 <= strs[i].length <= 200"],
      hints: ["Compare the first string with the second, find their common prefix, then compare that prefix with the third string, and so on."],
      ...
      commonMistakes: ["Using indexOf instead of checking strictly from index 0."],
      followUp: ["Can you do it using a Trie data structure?"]
    },
    {
      id: "dsa-code-20",
      ...
      hints: ["If the current sum becomes negative, it's better to start a new subarray from the next element."],
      ...
      commonMistakes: ["Returning 0 if all numbers are negative. The answer should be the maximum negative number."],
      followUp: ["Can you return the start and end indices of the max subarray?"]
    },
    {
      id: "dsa-code-21",
      ...
      examples: [{ input: "intervals = [[1,3],[2,6],[8,10],[15,18]]", output: "[[1,6],[8,10],[15,18]]", explanation: "Intervals [1,3] and [2,6] overlap, so they merge into [1,6]." }],
      ...
      commonMistakes: ["Not sorting the intervals first.", "Not taking the maximum of the end times when merging."],
      followUp: ["How would you insert a new interval into an already sorted list of non-overlapping intervals?"]
    },
    {
      id: "dsa-code-22",
      ...
      examples: [{ input: "s = \"(())\"", output: "true", explanation: "Correctly matched." }],
      constraints: ["1 <= s.length <= 10^4"],
      hints: ["You can simply keep a counter since there's only one type of bracket."],
      ...
      timeComplexity: "O(N)",
      spaceComplexity: "O(1)",
      solution: "function isValidSimple(s) {\n  let stack = [];\n  for(let char of s) {\n    if(char === '(') stack.push(char);\n    else { if(!stack.length) return false; stack.pop(); }\n  }\n  return stack.length === 0;\n}",
      optimizedSolution: "function isValidSimple(s) {\n  let count = 0;\n  for(let i=0; i<s.length; i++) {\n    if(s[i] === '(') count++;\n    else count--;\n    if(count < 0) return false;\n  }\n  return count === 0;\n}",
      commonMistakes: ["Returning true if the string is just '(' without checking count at the end."],
      followUp: ["What if you need to find the length of the longest valid parentheses substring?"]
    },
    {
      id: "dsa-code-23",
      title: "Rotate Array",
      ...
      hints: ["Try reversing the entire array, then reversing the first k elements, then the rest."],
      ...
      commonMistakes: ["Using unshift and splice repeatedly, resulting in O(N^2) complexity.", "Not handling k > array.length properly by using k % length."],
      followUp: ["Can you do this using cyclic replacements?"]
    },
    {
      id: "dsa-code-24",
      title: "Move Zeroes",
      ...
      examples: [{ input: "nums = [0,1,0,3,12]", output: "[1,3,12,0,0]", explanation: "Zeroes moved to end, order of 1,3,12 preserved." }],
      constraints: ["1 <= nums.length <= 10^4"],
      hints: ["Keep a pointer for the position where the next non-zero element should be written."],
      ...
      commonMistakes: ["Creating a new array, violating the in-place constraint.", "Using Array.splice() in a loop, causing O(N^2) complexity."],
      followUp: ["How can you minimize the total number of operations if the array has very few non-zero elements?"]
    },
    {
      id: "dsa-code-25",
      ...
      examples: [{ input: "nums = [5,4,-1,7,8]", output: "23", explanation: "The entire array has the max sum of 23." }],
      constraints: ["1 <= nums.length <= 10^5"],
      hints: ["Think about Kadane's algorithm, or try a divide and conquer approach."],
      ...
      optimizedSolution: "function maxSubArray(nums) {\n  // Divide and conquer approach (O(N log N)) for variety\n  function helper(arr, left, right) {\n    if (left === right) return arr[left];\n    let mid = Math.floor((left + right) / 2);\n    let leftMax = helper(arr, left, mid);\n    let rightMax = helper(arr, mid + 1, right);\n    let crossMax = crossSum(arr, left, right, mid);\n    return Math.max(leftMax, rightMax, crossMax);\n  }\n  function crossSum(arr, left, right, mid) {\n    let leftSum = -Infinity, currSum = 0;\n    for (let i = mid; i >= left; i--) {\n      currSum += arr[i]; leftSum = Math.max(leftSum, currSum);\n    }\n    let rightSum = -Infinity; currSum = 0;\n    for (let i = mid + 1; i <= right; i++) {\n      currSum += arr[i]; rightSum = Math.max(rightSum, currSum);\n    }\n    return leftSum + rightSum;\n  }\n  return helper(nums, 0, nums.length - 1);\n}",
      commonMistakes: ["Initializing max sum as 0 instead of the first element, which fails for all-negative arrays."],
      followUp: ["Can you print the actual subarray along with the sum?"]
    }
  ]
};
